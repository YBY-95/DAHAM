...
def main(test_name, dataset_name, dataset_subname, domain_name_s, domain_name_t, lamda,
    ...
    # 创建优化器
    f_optimizer = keras.optimizers.RMSprop(learning_rate=learning_rate)
    f_optimizer2 = keras.optimizers.RMSprop(learning_rate=0.1*learning_rate)
    c_optimizer = keras.optimizers.RMSprop(learning_rate=0.1*learning_rate)
    ...
    for epoch in range(epochs + 1):
        ...
        if epoch % 2 == 0:
            ...
            grad_f2 = tape.gradient(f_loss, feature.trainable_variables)
            f_optimizer2.apply_gradients(zip(grad_f2, feature.trainable_variables))
        ...
        if epoch % 50 == 0:
            ...
            # t_sne降维
            dir_fig = './picture' + '/' + test_name + '/' + dataset_subname + '/'+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
